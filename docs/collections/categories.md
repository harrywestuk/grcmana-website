...
| ------------ | --------------------------------------------------- | -------- |
| `categories` | self-referential (parent — via `nestedDocsPlugin`)  | no       |
...
> `parent` and `breadcrumbs` fields are injected by `nestedDocsPlugin` (configured in `src/plugins/index.ts`). They are not imported or defined in `Categories.ts`.
...
The following fields are injected by `nestedDocsPlugin` and must not be redefined in `Categories.ts`:

| Field         | Type                      | Source              | Notes                                                                                                                                           |
| ------------- | ------------------------- | ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| `parent`      | relationship → categories | `nestedDocsPlugin`  | Max depth of one enforced via `filterOptions` in the plugin config. Enables breadcrumb pattern: Articles > {Category} > {Sub-category} > {Slug} |
| `breadcrumbs` | array                     | `nestedDocsPlugin`  | Auto-populated from the parent chain. Used to build URLs and breadcrumb navigation                                                              |
...
| New fields           | `description`, SEO tab                                                       |
| `parent`             | Injected by `nestedDocsPlugin`. Add `filterOptions` depth-1 in plugin config |
...
- [x] `parent`: field injected by `nestedDocsPlugin` (already configured for `categories`). Max depth of one enforced via `filterOptions` in plugin config — not defined in `Categories.ts`. Supports breadcrumb pattern: Articles > {Category} > {Sub-category} > {Slug}.
...
| 2026-06-11 | `parent` delegated to `nestedDocsPlugin`; `filterOptions` in plugin config     |
